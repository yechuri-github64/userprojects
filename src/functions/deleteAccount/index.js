...
module.exports = async function (context, req) {
  context.log("HTTP trigger - Delete Account");
  if (!checkApiKey(req)) {
    context.log("Unauthorized request to delete account");
    context.res = {
      status: 401,
  ...
  try {
    ...
    context.log.error("Failed to delete account", err);
    context.res = {
      status: 500,
      headers: { "Content-Type": "application/json" },
      body: responses.error("Failed to delete account", "db_error"),
    };
  }
};
